...
`static/machine-store.js` เป็น shared browser state สำหรับ selected slot, speed X/Y/Z, position, homed state, connection, alarm, active command และ validation/arm state ส่วน `static/api-client.js` รับผิดชอบ HTTP/timeout/response parsing, `static/router.js` รับผิดชอบ hash/deep-link, `static/page-controllers.js` จำกัดอายุ timer/listener เฉพาะหน้าที่เปิด และ controller รายหน้า เช่น I/O, Events, Flow, MQTT และ Alarms ดูแล interaction เฉพาะ workspace นั้น `static/app.js` ยังคงรับผิดชอบ interaction/rendering ส่วนที่ยังไม่ย้าย ค่าใน localStorage เป็น preference เท่านั้น ไม่ใช่ machine authority
