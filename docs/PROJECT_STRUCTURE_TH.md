...
CSS โหลด `static/tokens.css` ก่อนเสมอเพื่อกำหนดสี semantic, typography, geometry และ global reset จากนั้นจึงโหลด `static/style.css` ซึ่งเป็น component/page layer การแยก page-specific CSS ที่เหลือดำเนินการแบบ incremental เพื่อลดความเสี่ยงจาก cascade regression
