...
CSS โหลดตามลำดับ `static/tokens.css` → `static/style.css` → `static/layout.css` โดยไฟล์แรกกำหนดสี semantic, typography, geometry และ global reset ไฟล์กลางเก็บ component styles เดิม และไฟล์สุดท้ายเป็น authority สำหรับ responsive shell และ page-specific layout การแยก component CSS ที่เหลือดำเนินการแบบ incremental เพื่อลดความเสี่ยงจาก cascade regression
