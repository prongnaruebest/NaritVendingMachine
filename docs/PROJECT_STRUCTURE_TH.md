...
CSS โหลดตามลำดับ `static/tokens.css` → `static/style.css` → `static/components.css` → `static/layout.css` โดยไฟล์แรกกำหนดสี semantic, typography, geometry และ global reset, `style.css` เก็บ legacy base/page styles ที่ยังไม่ย้าย, `components.css` เก็บ reusable controls (เริ่มจาก linked per-axis speed control) และไฟล์สุดท้ายเป็น authority สำหรับ responsive shell และ page-specific layout การแยก component CSS ที่เหลือดำเนินการแบบ incremental เพื่อลดความเสี่ยงจาก cascade regression
...
`static/motion-jog-safety-controller.js` ผูก pointer/keyboard hold-to-run และ fail-safe stop กับ pointer cancel, lost capture, window blur, page visibility และการออกจากหน้า Motion พร้อมถอด listener ทุกครั้งที่ unmount โดยไม่ถือ machine authority
