...
Wire checksum ใช้ SHA-256 ของ canonical ASCII fields ทุก field ยกเว้นตำแหน่ง checksum เอง ทั้ง Raspberry Pi serializer และ firmware parser ใช้กติกาเดียวกัน Firmware คำนวณ digest ใหม่ก่อน parse/stage และตอบ `COMMAND` error เมื่อข้อมูล phase, pulse, sensor หรือ watchdog ถูกแก้ระหว่างทาง Implementation ผ่าน known SHA-256 test vector และ tamper test แล้ว แต่ยังอยู่ใน candidate source เท่านั้น
