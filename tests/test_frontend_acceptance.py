...
class FrontendAcceptanceTests(unittest.TestCase):
    ...
    def test_io_status_separates_picontrol_from_iriv_modbus(self) -> None:
        ...
        self.assertIn('controllerIORegistry({ source: "picontrol_local", direction: "input" })', APP_JS)
        self.assertNotIn('key: "x_pend", channel: 2, pin: 27', APP_JS)
        self.assertNotIn('key: "y_pend", channel: 3, pin: 22', APP_JS)
        self.assertIn('channel.kind === "drive_alarm"', APP_JS)
        self.assertIn("Controller registry", APP_JS)
