...
class HealthApiTests(unittest.TestCase):
    ...
    def test_motion_page_exposes_operator_position_and_stop_controls(self) -> None:
        ...
        self.assertIn('{ requiredAxes: [axis], timeoutMs: 300000 }', app_js)
        self.assertIn("if (!window.confirm(confirmation)) return;", app_js)
        self.assertIn('travel-limit-x-sensors', html)
        self.assertIn("app.js') }}?v=20260907_home_visible_v7", html)
        self.assertIn("style.css') }}?v=20260907_home_visible_v7", html)
        self.assertIn('id="io-open-homing"', html)
        self.assertIn('id="homing-controls"', html)
        self.assertIn('id="nav-homing-controls"', html)
