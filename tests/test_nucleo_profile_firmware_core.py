...
def test_dynamic_protocol_enforces_revision_position_and_idempotency(tmp_path: Path):
    compiler = shutil.which("gcc")
    if compiler is None:
        pytest.skip("gcc is required for the firmware host harness")
    executable = tmp_path / "dynamic_protocol_test.exe"
    compile_result = subprocess.run(
        [
            compiler, "-std=c99", "-Wall", "-Wextra", "-Werror", f"-I{CORE}",
            str(CORE / "nucleo_dynamic_protocol.c"),
            str(ROOT / "tests" / "c_host" / "test_nucleo_dynamic_protocol.c"),
            "-o", str(executable),
        ],
    ...
    assert "dynamic protocol host tests passed" in run_result.stdout


def test_dynamic_coordinator_completes_xy_independently_and_stops_globally(tmp_path: Path):
    compiler = shutil.which("gcc")
    if compiler is None:
        pytest.skip("gcc is required for the firmware host harness")
    executable = tmp_path / "dynamic_coordinator_test.exe"
    sources = [
        CORE / "nucleo_virtual_kp.c", CORE / "nucleo_constraint_envelope.c",
        CORE / "nucleo_dynamic_planner.c", CORE / "nucleo_dynamic_runtime.c",
        CORE / "nucleo_dynamic_coordinator.c",
        ROOT / "tests" / "c_host" / "test_nucleo_dynamic_coordinator.c",
    ]
    compile_result = subprocess.run(
        [compiler, "-std=c99", "-Wall", "-Wextra", "-Werror", f"-I{CORE}",
    ...
    assert run_result.returncode == 0, run_result.stdout + run_result.stderr
    assert "dynamic coordinator host tests passed" in run_result.stdout


def test_dynamic_facade_binds_revision_targets_and_emitted_pulses(tmp_path: Path):
    compiler = shutil.which("gcc")
    if compiler is None:
        pytest.skip("gcc is required for the firmware host harness")
    executable = tmp_path / "dynamic_facade_test.exe"
    sources = [
        CORE / "nucleo_virtual_kp.c", CORE / "nucleo_constraint_envelope.c",
        CORE / "nucleo_dynamic_planner.c", CORE / "nucleo_dynamic_runtime.c",
        CORE / "nucleo_dynamic_coordinator.c", CORE / "nucleo_dynamic_protocol.c",
        CORE / "nucleo_dynamic_facade.c",
        ROOT / "tests" / "c_host" / "test_nucleo_dynamic_facade.c",
    ]
    compile_result = subprocess.run(
        [compiler, "-std=c99", "-Wall", "-Wextra", "-Werror", f"-I{CORE}",
         *(str(source) for source in sources), "-lm", "-o", str(executable)],
    ...
    assert run_result.returncode == 0, run_result.stdout + run_result.stderr
    assert "dynamic facade host tests passed" in run_result.stdout


@pytest.mark.parametrize("feature_enabled", [False, True])
def test_dynamic_dispatcher_is_bounded_prioritized_and_feature_gated(
    tmp_path: Path, feature_enabled: bool,
):
    compiler = shutil.which("gcc")
    if compiler is None:
        pytest.skip("gcc is required for the firmware host harness")
    # Avoid Windows compatibility-name heuristics that can mark binaries with
    # words such as "dispatcher" as requiring an elevated launch token.
    executable = tmp_path / f"dynrx_{int(feature_enabled)}.exe"
    sources = [
        CORE / "nucleo_virtual_kp.c", CORE / "nucleo_constraint_envelope.c",
        CORE / "nucleo_dynamic_planner.c", CORE / "nucleo_dynamic_runtime.c",
        CORE / "nucleo_dynamic_coordinator.c", CORE / "nucleo_dynamic_protocol.c",
...
def test_dynamic_runtime_prioritizes_stop_disarm_watchdog_and_deadline(tmp_path: Path):
    compiler = shutil.which("gcc")
    if compiler is None:
        pytest.skip("host GCC is unavailable")
    executable = tmp_path / "dynamic_runtime_test.exe"
    sources = [
        CORE / "nucleo_virtual_kp.c", CORE / "nucleo_constraint_envelope.c",
        CORE / "nucleo_dynamic_planner.c", CORE / "nucleo_dynamic_runtime.c",
        CORE / "nucleo_control_tick.c",
        ROOT / "tests" / "c_host" / "test_nucleo_dynamic_runtime.c",
    ]
    compile_result = subprocess.run(
        [compiler, "-std=c99", "-Wall", "-Wextra", "-Werror", f"-I{CORE}",
         *(str(source) for source in sources), "-lm", "-o", str(executable)],
    ...
    assert run_result.returncode == 0, run_result.stdout + run_result.stderr
    assert "dynamic runtime host tests passed" in run_result.stdout
...
def test_virtual_kp_is_xy_only_bounded_and_deterministic(tmp_path: Path):
    ...
    if compiler is None:
        pytest.skip("host GCC is unavailable")
    executable = tmp_path / "virtual_kp_test.exe"
    compile_result = subprocess.run(
        [
            compiler, "-std=c99", "-Wall", "-Wextra", "-Werror",
            f"-I{CORE}", str(CORE / "nucleo_virtual_kp.c"),
            str(ROOT / "tests" / "c_host" / "test_nucleo_virtual_kp.c"),
            "-o", str(executable),
        ], capture_output=True, text=True, timeout=30, check=False,
    )
    assert compile_result.returncode == 0, compile_result.stdout + compile_result.stderr
    ...
    assert run_result.returncode == 0, run_result.stdout + run_result.stderr
    assert "virtual Kp host tests passed" in run_result.stdout
...
def test_scurve_planner_handles_zero_short_long_and_reverse_moves(tmp_path: Path):
    ...
    if compiler is None:
        pytest.skip("host GCC is unavailable")
    executable = tmp_path / "scurve_planner_test.exe"
    compile_result = subprocess.run(
        [
            compiler,
            "-std=c99",
            ...
            "-Werror",
            f"-I{CORE}",
            str(CORE / "nucleo_scurve_planner.c"),
            str(ROOT / "tests" / "c_host" / "test_nucleo_scurve_planner.c"),
            "-lm",
            "-o",
            str(executable),
        ],
    ...
    assert run_result.returncode == 0, run_result.stdout + run_result.stderr
    assert "S-curve planner host tests passed" in run_result.stdout
