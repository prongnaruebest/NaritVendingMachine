...
class NucleoLink:
        ...
        with self._lock:
            ...
            try:
                ...
                for axis_name in participating:
                    ...
                    if (
                        axis_state != "COMPLETE"
                        or axis_fault not in ("NONE", "")
                        or axis_info.get("remaining_pulses") != 0
                    ):
                        raise NucleoError(
                            f"Dynamic move failed on axis {axis_name.upper()}: "
                            f"state={axis_state}, fault={axis_fault}, "
    ...
    def status_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "heartbeat_age_ms": payload.get("heartbeat_age_ms"),
            "max_heartbeat_gap_ms": payload.get("max_heartbeat_gap_ms"),
            "watchdog_trip_count": payload.get("watchdog_trip_count"),
            "uart_overrun_count": payload.get("uart_overrun_count"),
            "rx_dropped_bytes": payload.get("rx_dropped_bytes"),
            "moving": moving,
